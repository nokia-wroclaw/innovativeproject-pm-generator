...
import pandas as pd
import tsgm
from model_utils import cVAE_LSTMv3Architecture
...
def prepare_data(
    ...
    X = np.stack(groups).astype(np.float32)
...
def build_cvae_lstm_model(
    ...
    """Build and compile cVAE_LSTMv3Architecture + cBetaVAE. Returns (arch, model)."""
    arch = cVAE_LSTMv3Architecture(
        seq_len=seq_len,
        feat_dim=feat_dim,
...
def save_artifacts(
    ...
    np.save(
        out_dir / "window_anchors.npy",
        pd.to_datetime(data["window_anchors"]).to_numpy(dtype="datetime64[ns]"),
    )
...
def load_artifacts(
    ...
    model((dummy_X, dummy_y), training=False)  # builds the graph
...
def _inverse_transform(scaler, x_3d):
    try:
        return scaler.inverse_transform(x_3d)
    except Exception:
        n, t, f = x_3d.shape
        inv = scaler.inverse_transform(x_3d.reshape(n * t, f))
...
def _run_pipeline(model, X_windows, y_windows, scaler, rng, batch_size):
    ...
    return _inverse_transform(scaler, x_syn)
...
def generate_timespan(
    ...
    **_,
    ...
    x_real = _run_pipeline(model, X_m, y_m, scaler, rng, batch_size)
    n_windows, seq_len, n_kpis = x_real.shape
    anchors_arr = anchors_m.to_numpy()
    kpi_flat = x_real.reshape(n_windows * seq_len, n_kpis)
...
def generate_n_weeks(
    ...
    **_,
    ...
    x_real = _run_pipeline(model, X_pool[idx], y_pool[idx], scaler, rng, batch_size)
    n_w, seq_len, n_kpis = x_real.shape
    week_idx = np.repeat(np.arange(n_w), seq_len)
    hour_in_week = np.tile(np.arange(seq_len), n_w)
    kpi_flat = x_real.reshape(n_w * seq_len, n_kpis)
