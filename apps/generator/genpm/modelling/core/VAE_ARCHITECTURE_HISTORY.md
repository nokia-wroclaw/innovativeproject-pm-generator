...
## What was deleted from `core/architectures.py` (and is preserved only here)

- `VAE_LSTMArchitecture` (unconditional v1), `cVAE_LSTMArchitecture` (conditional v1)
- `cVAE_LSTMv2Architecture`, `cVAE_LSTMv3Architecture`, `cVAE_LSTMv4Architecture`,
  `cVAE_LSTMv5Architecture`
- `BetaVAE`, `cBetaVAE` — the non-hierarchical model classes these architectures paired
  with (superseded by `cBetaVAE_Hierarchical` / `cBetaVAE_Hierarchical_v7`)

None of these were referenced anywhere outside their own definitions (verified via a
repo-wide search including every notebook) at the time of deletion.

**Kept** (still wired into `core/model.py`'s builders, or shared by what is):
`cVAE_LSTMv6Architecture`, `cVAE_LSTMv7Architecture`, `cBetaVAE_Hierarchical`,
`cBetaVAE_Hierarchical_v7`, and the shared layers `CellConditioning`, `Sampling`,
`DualSampling`, `HourlyPositionalEncoding`, `CrossKPICorrelation`, `_funnel_schedule`.
