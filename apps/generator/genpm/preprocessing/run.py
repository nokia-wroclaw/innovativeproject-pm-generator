...
from genpm.preprocessing.logic import kpi_coverage
...
def run_preprocessing(sdm: SparkDataManager, preprocessing_cfg: PreprocessingConfig) -> None:
    ...
    # simple_reports_pivoted = preprocessing_logic.simple_reports_pivot(simple_reports_df_raw)

    # simple_report_grouping_cols = ("distname", "bts_id", "datetime")
    # cell_config = tuple([c for c in simple_reports_pivoted.columns if c not in simple_report_grouping_cols])

    # # GROUPING COLS DEFINITION FOR LATER ARGUMENTS
    # _GROUPING_COLS = ("distname", "bts_id", *cell_config)

    # pm_cm_df_long = preprocessing_logic.pm_and_reports_data_joined(pm_df_long, simple_reports_pivoted, cell_config)
    ...
    # pm_df_long_filled_gaps = kpi_coverage.fill_internal_gaps(pm_cm_df_long, _GROUPING_COLS, "start_time")
    ...
    # )
    # _log_pm_diag(
    #     "after categorize_kpi_with_definitions (join with kpi_definitions)", pm_df_long_pre_impute, _GROUPING_COLS
    # )
    ...
    #     f.countDistinct(*_GROUPING_COLS).alias("n_groups"),
    #     f.countDistinct("start_time").alias("n_unique_anchors"),
    # ).first()
    # n_cells = good_window_stats["n_groups"]
    # logger.info(
    ...
    # _log_window_diag("good_windows_candidates (post prefilter)", good_windows_candidates, _GROUPING_COLS)
    ...
    # scaler = SimpleMinMaxScaler(value_col="kpi_value", group_cols=["kpi_id", *_GROUPING_COLS])
    # params_df = scaler.fit(pm_df_long_imputed)
    # pm_df_long_scaled = scaler.transform(pm_df_long_imputed)
    ...
    _log_pm_diag("after attach_windows_index_to_pm", pm_df_long_indexed_winds, _GROUPING_COLS)

    # TODO: FIX PIVOT NOT THIS:
    pm_df_wide_indexed_winds = (
        pm_df_long_indexed_winds.groupBy(*_GROUPING_COLS, "window_anchor")
        .pivot("kpi_id")
