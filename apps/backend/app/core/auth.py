...
def get_keycloak_settings() -> KeycloakSettings:
    """Loads Keycloak settings from environment variables."""
...
def _extract_roles(payload: TokenPayload, client_id: str) -> set[str]:
    realm_roles = getattr(payload, "realm_access", {}).get("roles", [])
    client_roles = getattr(payload, "resource_access", {}).get(client_id, {}).get("roles", [])
...
def _verify_token_signature(token: str, settings: KeycloakSettings) -> dict[str, Any]:
    """Decodes the JWT token and verifies its signature using Keycloak JWKS."""
    try:
...
def _validate_token_claims(payload: dict[str, Any], settings: KeycloakSettings) -> None:
...
def require_auth(payload: TokenPayload = Depends(get_token_payload)) -> TokenPayload:
    settings = get_keycloak_settings()
...
def require_admin(payload: TokenPayload = Depends(require_auth)) -> TokenPayload:
    settings = get_keycloak_settings()
    token_roles = _extract_roles(payload, settings.client_id)
...
def assert_modeling_admin(payload: TokenPayload) -> None:
    settings = get_keycloak_settings()
    token_roles = _extract_roles(payload, settings.client_id)
    if settings.admin_role not in token_roles:
